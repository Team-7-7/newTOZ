...
export class RegistrationScene extends Phaser.Scene {
  constructor() {
    super({ key: CST.SCENES.REGISTRATION })
  }
  ...
  create() {
    ...
    element.on('click', (event) => {
      ...
      if (event.target.name === 'loginButton') {
        ...
        // const inputEmail = element.getChildByID('email');
        ...
        if (inputUsername.value !== '' && inputPassword.value !== '' && inputCharacterName.value !== '') {
          const registrationLogic = async () => {
            try {
              //register the user and set token
              const { data: authResponse } = await axios.post("/auth/register", {
